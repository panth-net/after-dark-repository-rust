...
- **SDK and programmer's manuals** (`reference/sdk/`) — `ADM3SD.SIT` (the AD
  ...
  This is the primary specification for the module ABI.
...
disk via the File Manager, and there's no file system here. A real
emulator with a real After Dark 3.0
...
simply wouldn't arrive, and any input dismisses a saver regardless.
...
and Apple's; the player imports them at run time from the disk the user
supplies, and nothing in the repository or the packaged app holds any of it.

Which modules the list *offers* is a separate question from what ships: the
`DOES_NOT_PLAY` list in `crates/ad-player/src/library.rs` is applied when the
list is drawn, and `the_skip_lists_match_the_survey_baseline` fails if that
list and `tests/baseline/modules.json` ever disagree, so it cannot rot.

`dist/` is not committed. It's where the packaging scripts leave their output
locally; the downloads are GitHub Release assets, built from a version tag by
