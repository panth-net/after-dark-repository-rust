...
mod tests {
    ...
    fn a_tick_far_in_the_future_does_not_hold_the_window_shut() {
        ...
        let waited = before.elapsed();
        // The slack is deliberately wide, and wide is not weak here. Without the
        // clamp this waits ten seconds; the bound below is 25x under that, so the
        // regression it guards cannot slip past. What the slack absorbs is
        // `thread::sleep` overshooting its request on a loaded machine — a shared
        // CI runner took 162 ms for the 100 ms sleep, which said nothing about the
        // code. Asserting the scheduler's precision only produces a test that
        // fails for reasons no one can fix.
        assert!(
            waited <= MAX_SLEEP * 4,
    ...
    fn the_schedule_is_absolute_so_it_cannot_drift_slow() {
        ...
        let waited = before.elapsed();
        // The lower bound is the half that discriminates: it fails if the ticks
        // are not paced at all. The upper bound only catches a schedule that
        // drifts badly — six sleeps' worth of accumulated overshoot — so it is
        // set clear of what a loaded runner adds to six ordinary sleeps rather
        // than tight against the ideal 100 ms.
        assert!(
    ...
    fn a_long_stall_abandons_the_backlog_instead_of_racing() {
        ...
        assert_eq!(
            p.resyncs(),
            1,
            "the backlog must be dropped, not worked off"
        );
