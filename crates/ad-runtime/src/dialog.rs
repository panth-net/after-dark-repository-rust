...
/// Strip everything that could break out of the string literal this text is
/// about to be interpolated into.
///
/// `"` and `\` would end or escape the literal in AppleScript and in
/// PowerShell alike. `$` and `` ` `` matter for PowerShell alone, where a
/// double-quoted string expands `$(...)` as a *subexpression* — it runs it —
/// and a backtick escapes the character after it.
///
/// That is not theoretical here. These strings carry text nobody in this
/// project wrote: a failed import reports the name of the file it was handed
/// (see the `Err` arm in the player's first-run setup), so a file named
/// `$(...)` reaching a Windows message box would otherwise execute. Dropping
/// the characters rather than escaping them per platform keeps one rule for
/// all three, and nothing that belongs in a dialog needs any of them.
...
mod tests {
    ...
    fn quotes_and_backslashes_are_stripped_from_a_prompt() {
        let clean = sanitise("Save \"here\\there\" now");
        assert_eq!(clean, "Save herethere now");
        assert!(!clean.contains('"') && !clean.contains('\\'));
    }

    /// PowerShell expands `$(...)` inside a double-quoted string by *running*
    /// it, and a backtick escapes the next character. Both reach a Windows
    /// message box through text this project does not author: a failed import
    /// reports the name of the file it was given, so a file called
    /// `$(calc.exe).img` would otherwise launch it.
    #[test]
    fn powershell_expansion_characters_cannot_survive_into_a_dialog() {
        let clean = sanitise("Could not read $(calc.exe).img");
        assert_eq!(clean, "Could not read (calc.exe).img");
        assert!(!clean.contains('$'));

        assert!(!sanitise("a `b").contains('`'));
        // The whole set, in one string that tries every escape at once.
        let hostile = sanitise(r#"$(x) `y "z" \w ${env:PATH}"#);
