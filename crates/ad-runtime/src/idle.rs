...
//! plug-in has received **no keyboard events at all** since Catalina (see
//! `docs/LEARNINGS.md`), so an interactive module cannot be played from one — and Lunatic
...
impl IdleWatch {
    ...
    pub fn idle_for(&mut self, minutes: u32) -> bool {
        ...
        self.seconds()
            .is_some_and(|s| u64::from(s) >= want.max(1))
    }
...
/// on disk, and there is no file system here (see `docs/module-findings.md`).
...
mod tests {
    ...
    fn the_delay_cycles_through_the_offered_choices_and_wraps() {
        let mut s = IdleSettings { after_minutes: 1, ..IdleSettings::default() };
    ...
    fn an_unknown_delay_cycles_to_the_first_choice_rather_than_sticking() {
        let mut s = IdleSettings { after_minutes: 7, ..IdleSettings::default() };
        s.cycle_delay();
