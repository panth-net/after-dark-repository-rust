...
use ad_resource::{ResourceFork, hfs};
...
pub fn install_into(source: &Path, dest: &Path) -> Result<Installed, String> {
    ...
    std::fs::create_dir_all(&staging)
        .map_err(|e| format!("Could not write to {}: {e}", staging.display()))?;
...
fn is_module(bytes: &[u8]) -> bool {
    ResourceFork::parse(bytes).is_ok_and(|fork| fork.all().iter().any(|r| &r.res_type == b"ADgm"))
}
...
mod tests {
    ...
    fn the_library_is_outside_the_application_bundle() {
        ...
        assert!(
            !text.contains(".app/"),
            "library must not be inside a bundle: {text}"
        );
    ...
    fn a_real_disk_image_imports_into_a_usable_library() {
        ...
        assert!(
            count.modules > 50,
            "expected the full module set, got {count:?}"
        );
    ...
    fn an_import_with_no_font_is_refused_and_leaves_nothing_behind() {
        ...
        assert!(
            !dest.exists(),
            "a refused import must not create the library"
        );
        assert!(
            !dest.with_extension("incoming").exists(),
