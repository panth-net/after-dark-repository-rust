...
mod tests {
    ...
    fn a_second_save_replaces_the_first_and_leaves_no_temp_file() {
        ...
        sink.persist(&[stored(b"LFhs", 1, None, &[2, 2, 2])])
            .unwrap();
    ...
    fn the_previous_save_survives_a_failed_write() {
        ...
        let err = sink
            .persist(&[
                stored(b"LFhs", 1, None, &[8]),
                stored(b"LFhs", 1, None, &[9]),
            ])
...
fn import_into(from: &Path, to: &Path) -> Result<Imported, String> {
    ...
    for path in files {
        ...
        if let Err(e) = ResourceFork::parse(&bytes) {
            report
                .rejected
                .push(format!("{shown}: not a saved score ({e})"));
            continue;
        }
        let dest = to.join(&name);
...
mod export_tests {
    ...
    fn importing_accepts_real_saves_and_says_which_it_replaced() {
        ...
        std::fs::write(
            from.join("Lunatic Fringe.save.rsrc"),
            a_real_save(&[1, 2, 3]),
        )
        .unwrap();
        ...
        assert!(
            report.rejected[0].starts_with("Broken.save.rsrc"),
            "{:?}",
            report.rejected
        );
        ...
        assert_eq!(
            back[0].data,
            vec![1, 2, 3],
            "the backup's score, not the old one"
        );
        assert!(
            !to.join("notes.txt").exists(),
            "only .rsrc files are considered"
        );
