...
//! save uses (see `docs/LEARNINGS.md`), against a real [`ForkSink`] on a real directory.
...
fn a_high_score_written_through_the_traps_survives_a_fresh_runtime() {
    ...
    assert!(sink.path().exists(), "{} was not written", sink.path().display());
...
fn releasing_a_resource_is_not_a_durable_change_even_if_its_bytes_differ() {
    ...
    let block = tb.mem.deref_handle(h).expect("deref");
    tb.mem.write_bytes(block, &[0x4E, 0x71, 0x4E, 0x75, 0, 0, 0, 0]);
