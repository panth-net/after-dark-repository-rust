...
//! statically: a naive scan of the module binaries reports 653 candidate A-line
...
/// `movea.l $6(a3),a0 ; jsr (a0)`). The host therefore needs
...
/// After Dark glue in the modules themselves: all are Pascal —
...
impl Toolbox {
    ...
    pub fn new() -> Self {
        ...
        let trap_table_base = mem.reserve_host((TRAP_TABLE_ENTRIES + 1) * 2, "trap address table");
    ...
    pub fn blank_screen(&mut self) {
        ...
        self.qd
            .fb
            .clear(&mut self.mem, base, quickdraw::BLACK_INDEX);
    }
    ...
    fn answer_no_event(&mut self, evt: u32) {
        ...
        if msg == ad_detect::MAGIC_REQUEST {
            ...
            self.mem.write_u32(
                evt.wrapping_add(ad_detect::EVT_MESSAGE),
                ad_detect::MAGIC_REPLY,
            );
            self.mem
                .write_u32(evt.wrapping_add(ad_detect::EVT_WHERE), info);
            return;
        }
    ...
    // The cost of this change, recorded rather than hidden:
    ...
    fn dispose_port_regions(&mut self, port_addr: u32) {
        ...
        let keep = [
            self.qd.blank_rgn,
            self.mem
                .read_u32(self.screen.port.wrapping_add(port::port::VIS_RGN)),
            self.mem
                .read_u32(self.screen.port.wrapping_add(port::port::CLIP_RGN)),
        ];
    ...
    fn dispatch_os(&mut self, t: Trap, regs: &mut dyn Registers) -> Result<(), TrapError> {
        ...
        match key {
            ...
            0xA024 => {
                ...
                let err = if self.mem.resize_handle(h, want) {
                    0
                } else {
                    oserr::MEM_FULL_ERR
                };
            ...
            0xA046 | 0xA146 | 0xA346 | 0xA746 => {
                ...
                let addr = if let Some(&patched) = self.trap_patches.get(&canonical) {
                    ...
                    self.trap_table_base.wrapping_add(TRAP_TABLE_ENTRIES * 2)
                } else {
        ...
        if (callout::WORD_BASE..callout::WORD_BASE + callout::SLOTS).contains(&t.word) {
            ...
            match kind {
                ...
                Callout::SndLength => {
                    ...
                    let h = arg(&mut self.mem, 0);
                    let ticks = self.decode_snd_handle(h).map_or(30, |(_, s)| s.ticks());
                ...
                Callout::Discover => {
                    ...
                    return Err(TrapError {
                        trap: t.word,
                        detail: format!("host callout [{name}] invoked; stack: {}", args.join(" ")),
                    });
                }
            }
            return Ok(());
            ...
            0xA9A8 => {
                ...
                if let Some((e, ty, id)) = self.resources.info_for(h) {
                    ...
                    for (i, b) in bytes.iter().take(n).enumerate() {
                        self.mem.write_u8(name_out.wrapping_add(1 + i as u32), *b);
                    }
            ...
            0xAA1E => {
                ...
                let h = match bytes {
                    Some(b) => {
                        let h = self
                            .mem
                            .new_handle(u32::try_from(b.len()).unwrap_or(0).max(1), false);
                        if let Some(block) = self.mem.deref_handle(h) {
                    ...
                    if let Some(block) = self.mem.deref_handle(h) {
                        ...
                        for (i, c) in palette.iter().enumerate() {
                            ...
                            for (j, v) in c.iter().enumerate() {
                                ...
                                self.mem.write_u16(spec.wrapping_add(2 + (j as u32) * 2), w);
                            }
                        }
                    }
                    globals::LowMem::set_res_err(&mut self.mem, 0);
            ...
            0xA9EE => {
                ...
                match selector {
                    ...
                    0 => {
                        ...
                        self.mem
                            .write_u8(dst, u8::try_from(text.len()).unwrap_or(255));
