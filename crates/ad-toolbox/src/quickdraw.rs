...
impl QuickDraw {
    ...
    fn fill_poly(&self, mem: &mut Memory, pts: &[(i16, i16)], pat: &[u8; 8], fore: u8, back: u8) {
        if pts.len() < 3 {
            return;
        }
        let dst = self.dest(mem);
    ...
    #[allow(
        ...
        reason = "one parameter per QuickDraw verb"
    )]
    fn arc(
        &self,
        mem: &mut Memory,
            ...
            for bit in 0..g.bits {
                for row in 0..font.rect_height {
                    if font.strike_bit(g.strike_bit.saturating_add(bit), row) {
                        points.push((
                            pen.saturating_add(i32::from(g.left))
    ...
    fn draw_text_bytes(&mut self, mem: &mut Memory, bytes: &[u8]) {
        ...
        if points.is_empty() && advance == 0 {
            ...
            self.pen_h = self
                .pen_h
                .saturating_add(i16::try_from(bytes.len().saturating_mul(6)).unwrap_or(i16::MAX));
            return;
        }
        let dst = self.dest(mem);
    ...
    fn dispatch_inner(
        ...
        match t.canonical() {
            ...
            0xA892 => {
                ...
                let dv = s.pop_i16(mem);
                let dh = s.pop_i16(mem);
                let (h, v) = (self.pen_h.saturating_add(dh), self.pen_v.saturating_add(dv));
                if self.open_poly.is_some() {
                    self.poly_points.push((h, v));
                } else {
                    let ink = Ink::Pattern(self.pen_pat, self.fore, self.back);
            ...
            0xA817 => {
                ...
                crate::blit::copy_mask(
                    mem, &src, &mask, &dst, &src_rect, &mask_rect, &dst_rect, self.fore, self.back,
                    &to_index,
                );
            }
