...
impl PictState {
    ...
    fn map_pt(&self, h: i16, v: i16) -> (i32, i32) {
        let (sw, sh) = (self.src_frame.width().max(1), self.src_frame.height().max(1));
...
impl Reader<'_> {
    ...
    fn rect(&mut self) -> Result<Rect, PictError> {
        Ok(Rect::new(self.i16()?, self.i16()?, self.i16()?, self.i16()?))
    }
...
mod tests {
    ...
    fn draws_a_filled_rect_and_maps_coordinates() {
        ...
        draw_picture(
            &mut m, at, len, &dst, &Rect::new(0, 0, 32, 32),
            200, 0, 0x0042_0000, 0x1000, &ident,
        )
        .expect("draw");
    ...
    fn rgb_foreground_changes_the_colour_used() {
        ...
        let len = pict_v2(&mut m, at, Rect::new(0, 0, 16, 16), &body);
        draw_picture(
            &mut m, at, len, &dst, &Rect::new(0, 0, 16, 16),
            1, 0, 0x0044_0000, 0x1000, &ident,
        )
        .expect("draw");
        assert_eq!(
            dst.get(&mut m, 5, 5),
    ...
    fn same_rect_variants_reuse_the_previous_rectangle() {
        ...
        draw_picture(
            &mut m, at, len, &dst, &Rect::new(0, 0, 16, 16),
            200, 9, 0x0046_0000, 0x1000, &ident,
        )
        .expect("draw");
    ...
    fn unknown_opcode_stops_rather_than_desynchronising() {
        ...
        let err = draw_picture(
            &mut m, at, len, &dst, &Rect::new(0, 0, 16, 16),
            1, 0, 0x0048_0000, 0x1000, &ident,
        )
        .expect_err("must refuse");
        assert!(matches!(err, PictError::UnknownOpcode { opcode: 0x0077, .. }));
    }
    ...
    fn draws_an_embedded_one_bit_bitmap() {
        ...
        draw_picture(
            &mut m, at, len, &dst, &Rect::new(0, 0, 2, 8),
            200, 7, 0x004B_0000, 0x1000, &ident,
        )
        .expect("draw");
    ...
    fn a_version_one_version_opcode_carries_one_data_byte() {
        ...
        m.write_bytes(at, &bytes);
        draw_picture(
            &mut m, at, bytes.len() as u32, &dst, &Rect::new(0, 0, 16, 16),
            77, 0, 0x0053_0000, 0x1000, &ident,
        )
    ...
    fn a_version_one_picture_uses_single_byte_opcodes() {
        ...
        draw_picture(
            &mut m, at, bytes.len() as u32, &dst, &Rect::new(0, 0, 16, 16),
            123, 0, 0x004F_0000, 0x1000, &ident,
        )
        .expect("draw");
