...
//! software floating point, and a static scan found FP68K
...
pub fn fp68k(mem: &mut Memory, opword: u16, dst: u32, src: Option<u32>) -> Option<SaneResult> {
    ...
    match operation {
        ...
        envop::CPYSGN => {
            ...
            let out = if sv.is_sign_negative() {
                -d.abs()
            } else {
                d.abs()
            };
...
mod tests {
    ...
    fn extended_round_trips_exactly_for_representable_values() {
        ...
        for v in [
            0.0, 1.0, -1.0, 2.0, 0.5, -0.25, 3.0, 1024.0, -4096.0, 1e10, -1e-10, 0.1, 12345.678,
        ] {
