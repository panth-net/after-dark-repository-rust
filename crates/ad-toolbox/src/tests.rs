...
use super::*;
use crate::quickdraw::{map_rect, Rect, SCREEN_HEIGHT, SCREEN_WIDTH};
...
fn map_rect_scales_between_coordinate_spaces() {
    ...
    assert_eq!(map_rect(&Rect::new(3, 4, 5, 6), &src, &src), Rect::new(3, 4, 5, 6));
}
...
fn an_auto_pop_trap_finds_its_arguments_and_returns_to_the_caller() {
    ...
    let mut regs = FakeRegs::new(sp);
    tb.trap(0xAC2E, &mut regs).expect("auto-pop Colour Utilities");
    ...
    assert_eq!(regs.sp(), SP, "auto-pop must pop everything the glue pushed");
...
fn clipping_can_never_reach_after_darks_blank_region() {
    ...
    assert_eq!(tb.mem.read_u16(regs.sp()), 0, "blankRgn must not read as empty");
...
fn the_event_manager_block_is_where_the_modules_say_it_is() {
    ...
    for i in 0..16u32 {
        assert_eq!(tb.mem.read_u8(out + i), 0xA0 | i as u8, "$A976 KeyMap byte {i}");
    }
    ...
    assert_eq!(tb.mem.read_u16(out + 2), 123, "$A972 GetMouse writes h second");
    ...
    for trap in [0xA970u16, 0xA971] {
        ...
        assert_eq!(tb.mem.read_u16(regs.sp()), 0, "{trap:#06x} must report no event");
        assert_eq!(tb.mem.read_u16(out), 0, "{trap:#06x} leaves a nullEvent");
        assert_eq!(tb.mem.read_u32(out + 6), 7, "{trap:#06x} fills in `when`");
        assert_eq!(tb.mem.read_u16(out + 10), 45, "{trap:#06x} fills in `where.v`");
    }
...
fn boolean_results_live_in_the_high_byte() {
    ...
    assert_eq!(
        tb.mem.read_u8(slot), 1,
        "true must be in the high byte"
    );
...
fn a_sane_comparison_publishes_condition_codes() {
    ...
    let compare = |left: f64, right: i16| -> (u8, u32) {
        ...
        (
            regs.condition_codes().expect("a comparison must set the CCR"),
            regs.data(0),
        )
    };
    ...
    assert!(bgt(greater) && !blt(greater) && !beq(greater), "{greater:#04x}");
    assert_eq!(d0, 1, "D0 mirrors the ordering for glue that tests a register");
    ...
    assert!(!bgt(unordered), "a NaN must not satisfy bgt: {unordered:#04x}");
    ...
    assert_eq!(sane::comparison_ccr(sane::SaneResult::Compared(Ordering::Greater)), 0);
}
...
fn high_scores_survive_the_rmve_add_release_cycle() {
    ...
    tb.resources
        .add(&mut tb.mem, *b"LFhs", 128, None, new);
...
fn hsv_to_rgb_matches_the_wheel_at_the_primaries() {
    ...
    let close = |got: [u16; 3], want: [u16; 3], what: &str| {
        for i in 0..3 {
            ...
            assert!(d <= NEAR, "{what}: channel {i} was {got:?}, wanted {want:?}");
        }
    };
    close(hsv_to_rgb([0, 0xFFFF, 0xFFFF]), [0xFFFF, 0, 0], "red at hue 0");
    close(
...
fn plot_icon_scales_a_32x32_one_bit_icon_into_its_rect() {
    ...
    assert_eq!(regs.sp(), SP, "PlotIcon takes a rect and a handle, nothing back");

    let s = tb.qd.screen_surface();
    assert_eq!(s.get(&mut tb.mem, 12, 12), Some(200), "set bits use the fore colour");
    assert_eq!(s.get(&mut tb.mem, 40, 12), Some(5), "clear bits are srcCopy'd as back");
    assert_eq!(s.get(&mut tb.mem, 5, 5), Some(0), "outside the rect is untouched");
}
...
fn plot_cicon_draws_only_where_the_mask_allows() {
    ...
    let s = tb.qd.screen_surface();
    assert_eq!(s.get(&mut tb.mem, 4, 4), Some(red), "masked in, colour from the cicn table");
    assert_eq!(s.get(&mut tb.mem, 7, 4), Some(0), "masked out: destination shows through");
}
...
fn get_cicon_is_independent_of_the_resource_it_came_from() {
    ...
    assert_ne!(icon, resource, "GetCIcon must not hand back the resource handle");

    tb.resources.release(&mut tb.mem, resource);
    let block = tb.mem.deref_handle(icon).expect("the icon outlives the release");
...
fn a_pixel_pattern_carries_the_colour_make_rgb_pat_was_given() {
    ...
    assert_eq!(tb.mem.read_u16(block), 1, "patType starts as a colour pattern");
    ...
    assert_eq!(regs.sp(), SP);
    assert_eq!(tb.qd.back, want, "BackPixPat does the same for the background");
...
fn a_menu_is_walked_for_its_count_its_item_text_and_its_styles() {
    ...
    assert_eq!(regs.sp(), SP, "GetItem is ten bytes of arguments, no result");
...
fn every_file_manager_call_reports_that_no_volume_is_mounted() {
    ...
    for word in [0xA200u16, 0xA20A] {
        ...
        assert_eq!(tb.mem.read_u16(pb + filemgr::IO_RESULT) as i16, oserr::NSV_ERR);
    }
    ...
    let err = tb.trap(0xA260, &mut regs).expect_err("must name the selector");
...
fn an_installed_search_proc_makes_the_red_channel_a_palette_index() {
    ...
    tb.mem.write_u16(rgb, 137);
    call(&mut tb, 0xAA14, &[Arg::L(rgb)]);
    assert_ne!(tb.qd.fore, 137, "removing the proc restores colour matching");
}
...
fn add_search_refuses_a_second_different_procedure() {
    ...
    assert!(err.detail.contains("second search procedure"), "{}", err.detail);
}
...
fn key_translate_returns_the_character_for_a_key_code() {
    ...
    let translate = |tb: &mut Toolbox, code: u16| -> u8 {
        ...
        assert_eq!(regs.sp(), SP - 4, "the LONGINT result must be left on the stack");
    ...
    assert_eq!(translate(&mut tb, 0x25 | 0x80), b'l', "key-up still translates");
