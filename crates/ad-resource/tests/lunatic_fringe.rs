...
//! `AfterDark-original.img` (see `docs/LEARNINGS.md`) or comes from Berkeley
//! Systems' own SDK. If the parser drifts, these fail.

#![allow(clippy::indexing_slicing, clippy::arithmetic_side_effects)]

use ad_resource::{
    settings::{is_settings_type, CONTROL_ID_BASE},
...
fn parses_all_109_resources_with_expected_type_counts() {
    ...
    for (ty, want) in [
        ...
        assert_eq!(fork.count_of(ty), want, "count of {:?}", String::from_utf8_lossy(ty));
    }
...
fn code_header_and_entry_point_resolve() {
    ...
    let header = layout.header.expect("Lunatic Fringe has the 16-byte ADgm header");
    assert_eq!(
...
fn segment_jump_table_chain_is_intact() {
    ...
    assert_eq!(segs.iter().map(|r| r.id).collect::<Vec<_>>(), vec![-2045, -2044, -2043]);
...
fn segment_chain_arithmetic() {
    ...
    let a = SegmentHeader { jump_table_offset: 384, entry_count: 33 };
    assert_eq!(a.table_bytes(), 264);
    assert_eq!(a.next_offset(), 648);

    let b = SegmentHeader { jump_table_offset: 648, entry_count: 17 };
    assert_eq!(b.next_offset(), 784);

    // Clock's six-segment chain, also measured from the disk.
    let clock = [(680u16, 19u16), (832, 4), (864, 4), (896, 11), (984, 7), (1040, 7)];
    for w in clock.windows(2) {
        let (off, count) = w[0];
        let (next_off, _) = w[1];
        let h = SegmentHeader { jump_table_offset: off, entry_count: count };
        assert_eq!(
            h.next_offset(),
            u32::from(next_off),
    ...
    let max = SegmentHeader { jump_table_offset: u16::MAX, entry_count: u16::MAX };
...
fn segment_header_rejects_short_data() {
    ...
    assert_eq!(
        ...
        Some(SegmentHeader { jump_table_offset: 384, entry_count: 33 })
    );
}
...
fn settings_decode_matches_the_sdk_spec() {
    ...
    match slot3 {
        Control::Slider { label, value, units } => {
...
fn settings_types_are_recognised() {
    ...
    assert!(is_settings_type(b"\xB5Val"), "micro-sign type must be recognised");
...
/// its own loader and consumer code (see `docs/LEARNINGS.md`).
///
...
fn key_configuration_decodes_to_the_shipped_bindings() {
    ...
    assert_eq!(entry(0, 1), (0, b'6', 0x58, 0), "rotate other way: keypad 6");
