...
use ad_resource::{BitmapFont, ResourceFork, font::font_id_parts};
...
fn the_system_files_fonts_decode_into_legible_glyphs() {
    ...
    for r in strikes {
        ...
        for ch in b'!'..=b'~' {
            let g = font
                .glyph(ch)
                .unwrap_or_else(|| panic!("FONT {}: {ch}", r.id));
            assert!(
                g.advance > 0,
                "FONT {}: {:?} has no advance",
                r.id,
            ...
            if (0..g.bits)
                .any(|b| (0..font.rect_height).any(|y| font.strike_bit(g.strike_bit + b, y)))
            {
                inked += 1;
            }
        }
        ...
        assert!(
            (0..space.bits)
                .all(|b| (0..font.rect_height).all(|y| !font.strike_bit(space.strike_bit + b, y))),
