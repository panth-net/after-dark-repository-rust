...
impl<'a> Reader<'a> {
    ...
    fn header(bytes: &'a [u8]) -> Result<Self> {
        ...
        Ok(Self {
            bytes: bytes
                .get(..HEADER_LEN)
                .ok_or(Error::TooShort { len: fork_len })?,
            base: 0,
            fork_len,
        })
    }
...
mod tests {
    ...
    fn offsets_cannot_resolve_into_the_fork_header() {
        ...
        let err =
            ResourceFork::parse(&s.bytes).expect_err("a 16-byte map has no field at +24 to read");
        assert!(
            matches!(
                err,
                Error::OutOfBounds {
    ...
    fn resource_length_past_the_data_area_is_rejected() {
        ...
        let err =
            ResourceFork::parse(&s.bytes).expect_err("must not extend a payload into the map");
        assert!(
            matches!(
                err,
