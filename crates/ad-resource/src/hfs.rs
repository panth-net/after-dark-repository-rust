...
impl Mdb {
    fn parse(volume: &[u8]) -> Result<Self> {
        let mdb = volume
            .get(MDB_OFFSET..)
            .ok_or(Error::HfsMalformed { what: "master directory block" })?;
        Ok(Self {
            ...
            extents_file_size: be_u32(mdb, 130).ok_or(Error::HfsMalformed { what: "drXTFlSize" })?,
            extents_first_extents: extent_record(
                take(mdb, 134, 12).ok_or(Error::HfsMalformed { what: "drXTExtRec" })?,
            ),
            catalog_file_size: be_u32(mdb, 146).ok_or(Error::HfsMalformed { what: "drCTFlSize" })?,
    ...
    fn read_fork_from(
        ...
        let malformed = Error::HfsMalformed { what: "fork extents" };
        if size == 0 {
            return Ok(Vec::new());
        }
        let block_size = self.alloc_block_size;
        if block_size == 0 {
            return Err(Error::HfsMalformed { what: "drAlBlkSiz is zero" });
        }
        ...
        for &(start, count) in first {
            have = have.checked_add(u32::from(count)).ok_or(malformed.clone())?;
        ...
        // lookup fails.
        while have < needed {
            let key = (cnid, resource_fork, u16::try_from(have).map_err(|_| malformed.clone())?);
...
fn leaf_records(tree: &[u8]) -> Result<Vec<&[u8]>> {
    let malformed = Error::HfsMalformed { what: "B-tree header" };
    ...
    loop {
        if !seen.insert(node) {
            return Err(Error::HfsMalformed { what: "B-tree leaf chain loops" });
        }
...
mod tests {
    ...
    fn a_buffer_with_no_volume_header_is_rejected() {
        ...
        assert_eq!(resource_forks(&[0u8; 64]).unwrap_err(), Error::NotAnHfsVolume);
        assert_eq!(
