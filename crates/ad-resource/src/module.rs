...
//! `GraphicsModule_Types.h` (see `reference/sdk/`).
//!
...
impl CodeLayout {
    ...
    pub fn detect(code: &[u8]) -> Self {
        let has_header =
            code.get(0..2) == Some(&HEADER_BRA[..]) && code.get(4..8) == Some(&TYPE_ADGM[..]);
        if !has_header {
            return Self {
                header: None,
                entry_offset: 0,
...
impl SegmentHeader {
    ...
    pub fn parse(data: &[u8]) -> Option<Self> {
        Some(Self {
            jump_table_offset: u16::from_be_bytes(<[u8; 2]>::try_from(data.get(0..2)?).ok()?),
