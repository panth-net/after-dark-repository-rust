...
impl<'a> BitmapFont<'a> {
    ...
    pub fn parse(data: &'a [u8]) -> Result<Self> {
        ...
        let field =
            |at: usize, what: &'static str| be_i16(data, at).ok_or_else(|| oob(what, at, 2, len));
        ...
        let ow_end = ow_start.saturating_add(loc_len).min(len);
        let ow = data
            .get(ow_start..ow_end)
...
mod tests {
    ...
    fn truncation_is_rejected_down_to_the_last_readable_entry() {
        ...
        for n in minimum..=d.len() {
            let f =
                BitmapFont::parse(&d[..n]).unwrap_or_else(|e| panic!("len {n} was rejected: {e}"));
