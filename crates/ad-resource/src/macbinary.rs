...
impl<'a> MacBinary<'a> {
    ...
    pub fn parse(bytes: &'a [u8]) -> Result<Self> {
        ...
        let name = header
            .get(2..2usize.saturating_add(name_len))
        ...
        let be32 = |off: usize| -> Result<usize> { Ok(u32::from_be_bytes(arr4(off)?) as usize) };
...
mod tests {
    ...
    fn round_trips_both_forks() {
        ...
        assert!(
            mb.data_fork.is_empty(),
            "After Dark modules have no data fork"
        );
    ...
    fn rejects_lying_fork_length() {
        ...
        assert!(matches!(MacBinary::parse(&raw), Err(Error::NotMacBinary)));
    }
