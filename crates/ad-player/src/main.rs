...
const KEY_MAP: &[(Key, u8)] = &[
    ...
    (Key::LeftSuper, 0x37), (Key::RightSuper, 0x37), (Key::F, 0x37),
    (Key::LeftShift, 0x38), (Key::RightShift, 0x38),
    ...
    (Key::LeftAlt, 0x3A), (Key::RightAlt, 0x3A),
    (Key::LeftCtrl, 0x3B), (Key::RightCtrl, 0x3B),
    // Keypad: Lunatic Fringe's primary controls.
    (Key::NumPadDot, 0x41), (Key::NumPadAsterisk, 0x43), (Key::NumPadPlus, 0x45),
    (Key::NumPadSlash, 0x4B), (Key::NumPadEnter, 0x4C), (Key::NumPadMinus, 0x4E),
...
const MODERN_MAP: &[(Key, u8)] = &[
    // Turning: either hand.
    (Key::A, lf::TURN_LEFT), (Key::Left, lf::TURN_LEFT),
    (Key::D, lf::TURN_RIGHT), (Key::Right, lf::TURN_RIGHT),
    // Forward is thrust.
    (Key::W, lf::THRUST), (Key::Up, lf::THRUST),
    (Key::J, lf::FIRE),
    (Key::L, lf::TURBO),
    (Key::K, lf::SHIELD),
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // is what somebody moving to a new machine actually wants.
    {
        ...
        if matches!(flag, Some("--export" | "--import")) {
            let Some(dir) = argv.get(1).map(PathBuf::from) else {
                eprintln!("{} needs a folder: ad-player {} <folder>", flag.unwrap_or(""), flag.unwrap_or(""));
                std::process::exit(2);
            };
    ...
    if let Some(path) = direct {
        return run_module(&path, &window, audio.as_ref(), &options, false, true).map_err(Into::into);
    }
...
fn default_library() -> PathBuf {
    ...
    if let Ok(exe) = std::env::current_exe() {
        ...
        if let Some(res) = exe.parent().and_then(Path::parent).map(|c| c.join("Resources")) {
...
fn screenshot(dir: &Path, file: &Path) -> Result<(), String> {
    ...
    let font = Font::discover(dir)
        .ok_or_else(|| format!("no Macintosh font in {}", dir.display()))?;
    ...
    let rows = draw_browser(&mut canvas, &font, &modules, selected, &mut scroll, None, &idle);
    reveal(selected, rows.visible, &mut scroll);
    draw_browser(&mut canvas, &font, &modules, selected, &mut scroll, None, &idle);
...
fn parse_lfhs(data: &[u8]) -> Vec<ScoreRow> {
    let mut rows: Vec<ScoreRow> = data
        ...
        .filter_map(|e| {
            ...
            let score =
                i32::from_be_bytes([*e.get(20)?, *e.get(21)?, *e.get(22)?, *e.get(23)?]);
            if score < 0 || len == 0 {
...
fn export_scores_interactively() -> String {
    let Some(dest) = ad_runtime::choose_folder("Choose a folder for your After Dark scores")
    else {
...
fn import_scores_interactively() -> String {
    ...
    let Some(src) =
        ad_runtime::choose_file_or_folder("Choose an exported After Dark scores file")
    else {
...
fn browse(
    ...
    let Some(font) = Font::discover(dir) else {
        println!("No Macintosh font found in {} (looked for System.rsrc and", dir.display());
    ...
    let mut rows = Rows {
        ...
        left: 0,
        width: 0,
        play_win: Rect { x: 0, y: 0, w: 0, h: 0 },
        play_full: Rect { x: 0, y: 0, w: 0, h: 0 },
        export: Rect { x: 0, y: 0, w: 0, h: 0 },
        import: Rect { x: 0, y: 0, w: 0, h: 0 },
        idle_toggle: Rect { x: 0, y: 0, w: 0, h: 0 },
        idle_module: Rect { x: 0, y: 0, w: 0, h: 0 },
        idle_delay: Rect { x: 0, y: 0, w: 0, h: 0 },
        idle_sound: Rect { x: 0, y: 0, w: 0, h: 0 },
        idle_preview: Rect { x: 0, y: 0, w: 0, h: 0 },
    };
...
fn draw_browser(
    ...
    *scroll = (*scroll).min(max_scroll);
    ...
    for slot in 0..visible {
        ...
        let ink = if chosen { colour::SELECTED_INK } else { colour::INK };
        canvas.text_clipped(
            &f,
            14,
    ...
    if let Some(entry) = modules.get(selected) {
        ...
        canvas.text(
            ...
            dx + 10,
            y,
            &format!("{} resources{}", entry.resources, if entry.has_sound { ", sound" } else { "" }),
            colour::DIM,
        );
        y += row_h + 6;
    ...
    let mut panel_button = |canvas: &mut Canvas, label: &str| -> Rect {
        ...
        let r = Rect { x: pb_x, y: pb_y, w: bw, h: pb_h };
        pb_x = r.x + bw + 8;
        canvas.rect(r.x, r.y, r.w, r.h, colour::BACKGROUND);
        canvas.frame(r.x, r.y, r.w, r.h, colour::FRAME);
        canvas.text(&f, r.x + 10, r.y + i32::from(f.ascent) + 4, label, colour::INK);
        r
    };
    ...
    let mut idle_button = |canvas: &mut Canvas, label: &str, on: bool, max_w: i32| -> Rect {
        ...
        let r = Rect { x: left, y: iby, w: bw, h: ih };
        left = r.x + bw + 6;
        let (fill, ink) = if on {
        ...
        canvas.frame(r.x, r.y, r.w, r.h, colour::FRAME);
        canvas.text_clipped(&f, r.x + 9, r.y + i32::from(f.ascent) + 3, label, bw - 14, ink);
        r
    };
    ...
    let idle_delay = idle_button(canvas, &format!("After: {}", idle.delay_label()), false, i32::MAX);
    let idle_sound = idle_button(
        canvas,
        if idle.sound { "Sound: on" } else { "Sound: muted" },
        false,
        i32::MAX,
    );
    ...
    let mut button = |canvas: &mut Canvas, label: &str| -> Rect {
        let bw = f.text_width(label.as_bytes()) + 20;
        let r = Rect { x: right - bw, y: by, w: bw, h: bh };
        right = r.x - 8;
        canvas.rect(r.x, r.y, r.w, r.h, colour::BACKGROUND);
        canvas.frame(r.x, r.y, r.w, r.h, colour::FRAME);
        canvas.text(&f, r.x + 10, r.y + i32::from(f.ascent) + 3, label, colour::INK);
        r
    };
...
fn run_module(
    ...
    window
        .borrow_mut()
        .set_input_callback(Box::new(KeyTaps { taps: Rc::clone(&taps) }));
            ...
            Box::new(move |fb, ticks| {
                ...
                if stats && !s.held.is_empty() {
                    let codes: Vec<String> =
                        s.held.iter().map(|c| format!("{c:#04X}")).collect();
                    eprintln!(
            ...
            if options.stats {
                ...
                if now.duration_since(loop_epoch) >= std::time::Duration::from_secs(1) {
                    ...
                    eprintln!(
                        ...
                        c.0 - prev.0, c.1 - prev.1, c.2 - prev.2,
...
mod tests {
    ...
    fn the_cursor_maps_through_the_letterbox() {
        ...
        assert_eq!(window_to_buffer((640, 480), (639.0, 479.0)), Some((639, 479)));

        // Exactly 2x, no letterbox: 4:3 into 4:3.
        assert_eq!(window_to_buffer((1280, 960), (0.0, 0.0)), Some((0, 0)));
        assert_eq!(window_to_buffer((1280, 960), (640.0, 480.0)), Some((320, 240)));
        assert_eq!(window_to_buffer((1280, 960), (1279.0, 959.0)), Some((639, 479)));
    ...
    fn the_modern_layout_replaces_a_key_rather_than_adding_to_it() {
        ...
        assert_eq!(code_for(Key::L, false, false), Some(0x25), "L turns left originally");
        ...
        let known = [
            lf::TURN_LEFT, lf::TURN_RIGHT, lf::THRUST,
            lf::TURBO, lf::SHIELD, lf::ABORT, lf::FIRE,
        ];
        for (key, code) in MODERN_MAP {
            assert!(known.contains(code), "{key:?} maps to {code:#04x}, which LFky does not name");
        }
    }
    ...
    fn only_a_request_for_typing_turns_typing_on() {
        ...
        assert!(asks_for_typing(&["High score!  Enter your name:".to_owned()]));
    ...
    fn the_reserved_keys_come_back_only_while_typing() {
        ...
        for (key, _) in RESERVED_KEYS {
            ...
            let is_own_code = held_back == RESERVED_KEYS.iter()
                .find(|(k, _)| k == key)
                .map(|(_, c)| *c);
    ...
    fn the_idle_choice_honours_the_name_and_survives_it_disappearing() {
        ...
        let random = IdleSettings { module: IdleModule::Random, ..IdleSettings::default() };
        for _ in 0..50 {
    ...
    fn the_high_score_table_reads_real_entries_and_skips_empty_slots() {
        ...
        assert_eq!(rows[0], ScoreRow { name: "ACES".into(), score: 152, level: 3 });
        assert_eq!(rows[1], ScoreRow { name: "AR".into(), score: 12, level: 1 });
    ...
    fn a_click_right_of_the_list_does_not_select_a_row() {
        ...
        assert_eq!(rows.row_at(400, 45), None, "the details panel is not the list");
    ...
    fn the_controls_banner_fits_the_screen() {
        ...
        for (turn, thrust, named) in [("A / D", "W", "WAD"), ("Left / Right", "Up", "arrow")] {
            assert!(
                MODERN_SETS.iter().any(|l| l.contains(turn) && l.contains(thrust))
    ...
    fn leaving_takes_two_presses_and_the_first_one_expires() {
        ...
        let mut quit = false;
        let press = |tick: u32, armed: &mut Option<u32>, quit: &mut bool| {
            match *armed {
        ...
        assert!(!quit, "an expired arming cannot be confirmed by a later press");
    ...
    fn reserved_keys_are_not_passed_through() {
        ...
        for key in [Key::Tab, Key::Enter, Key::G] {
            assert!(!CAPS_TOGGLE.contains(&key), "{key:?} is no longer a pause key");
        }
        assert_eq!(code_for(Key::Tab, false, false), Some(0x30), "Tab reaches modules again");
        assert_eq!(code_for(Key::Enter, false, false), Some(0x24), "Return does too");
    ...
    fn the_caps_latch_toggles_on_the_edge() {
        ...
        for down in [false, true, true, true, true, false, false, true, true, true] {
            if down && !was_down {
                caps = !caps;
            }
            was_down = down;
