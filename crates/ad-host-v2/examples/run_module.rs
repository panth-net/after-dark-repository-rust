...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let path = PathBuf::from(args.next().ok_or("usage: run_module <module.rsrc> [frames] [out.png]")?);
    ...
    // useful output.
    {
        ...
        println!("module    {}", path.file_stem().unwrap_or_default().to_string_lossy());
        ...
        if let Some(l) = module.code_layout() {
            println!(
                ...
                if l.resolved_via_stub { "resolved" } else { "fallback" }
            );
        }
    ...
    if let Some(dir) = options.save_dir.as_deref() {
        let saved = ad_runtime::ForkSink::load(dir, &title)
            .map_err(|e| format!("saved state: {e}"))?;
        println!("saves     {} ({} resource(s) restored)", dir.display(), saved.len());
    ...
    if let Ok(s) = std::env::var("AD_KEYS") {
        for part in s.split(',') {
            let Some((when, rest)) = part.split_once(':') else { continue };
            ...
            if let Some(tick) = when.strip_prefix('t') {
                if let Ok(t) = tick.parse() {
                    println!("key         {code:#04x} {} at tick {t}", if down { "down" } else { "up" });
    ...
    for i in 0..frames {
        for &(frame, code, down) in &key_script {
            if frame == i {
                println!("key         {code:#04x} {}", if down { "down" } else { "up" });
        ...
        let traced = std::env::var("AD_TRACE_FRAME").ok().and_then(|v| v.parse::<u32>().ok());
        if traced == Some(i) {
            host.set_trace(2048);
        }
...
fn dump_sounds(host: &Host) -> std::io::Result<()> {
    ...
    if !played.is_empty() {
        println!("sounds      {} played, WAVs in {}", played.len(), dir.display());
    }
    Ok(())
}
