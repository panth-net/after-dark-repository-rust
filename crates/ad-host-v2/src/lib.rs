...
//! Systems' `GraphicsModule_main.c` (see `reference/sdk/`).
//!
...
use ad_toolbox::{Toolbox, quickdraw};
...
impl Host {
    ...
    pub fn load(fork: ResourceFork<'_>, settings_values: [i16; 4]) -> Result<Self, HostError> {
        ...
        let error_message =
            Self::build_param_block(&mut tb, param_block, settings_values, qd_globals, monitors);
        ...
        tb.mem.write_u32(ad_m68k::VECTOR_LINE_A, ad_m68k::TRAP_GATE);
        ...
        cpu.set_wild_jump_ceiling(ad_memory::HOST_ARENA.saturating_add(ad_memory::HOST_ARENA_SIZE));
        ...
        if let Some(header) = packed {
            ...
            let plain =
                decompress::expand(&mut tb, &mut cpu, header, &packed_bytes, &dcmp, scratch)
    ...
    #[allow(
        ...
        reason = "one argument per GMParamBlock region"
    )]
    fn build_param_block(
        ...
        // After Dark extensions, with the ABI taken from the modules' own
        // glue: `LookUpEntryPoints` walks
        ...
        let ext_table = tb
            .mem
            .reserve_host(2 + 8 * services.len() as u32, "extension table");
        ...
        tb.mem
            .write_u32(pb.wrapping_add(off::SND_CHANNEL as u32), 0);
    ...
    pub fn error_message(&mut self) -> Option<String> {
        ...
        let bytes = self
            .tb
            .mem
            .read_bytes(self.error_message.wrapping_add(1), len);
    ...
    pub fn trace_tail(&mut self, count: usize) -> String {
        ...
        for pc in pcs.iter().skip(start) {
            ...
            let _ = writeln!(out, "    +{:#06x}  {text}", pc.wrapping_sub(CODE_BASE));
        }
        out
    }
    ...
    pub fn layout(&self) -> String {
        format!(
            ...
            self.param_block,
            self.storage_var,
            self.tb.qd.screen_base,
            self.tb.qd.blank_rgn,
            self.entry
        )
    }
    ...
    pub fn set_present_hook(&mut self, every: u32, f: PresentHook) {
