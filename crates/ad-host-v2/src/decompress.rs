    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MissingDecompressor(id) => {
                write!(
                    f,
                    "resource needs dcmp {id}, which the module does not carry"
                )
            }
...
pub fn expand(
    ...
    loop {
        ...
        if debug {
            let t = cpu.trace();
            eprintln!(
                "[dcmp] ran {ran} cycles, stop_hit={hit}, {} traced",
                t.len()
            );
            ...
            eprintln!(
                "[dcmp] first 40: {}",
                off.iter().take(40).cloned().collect::<Vec<_>>().join(" ")
            );
            eprintln!(
                "[dcmp] last 40:  {}",
                off.iter()
    ...
    let out = tb.mem.read_bytes(dst, header.unpacked_len as usize);
    if std::env::var_os("AD_DCMP_DEBUG").is_some() {
            ...
            match (nz, run_start) {
                ...
                (false, Some(st)) => {
                    if a - st > 8 && reported < 12 {
                        let what = if st >= src && st < src + src_len {
                            "SRC"
                        } else if st >= dst && st < dst + header.unpacked_len {
                            "DST"
                        } else if st >= work && st < dcmp_at {
                            "WORK"
                        } else if st >= dcmp_at && st < stack {
                            "DCMP"
                        } else {
                            "other"
                        };
        ...
        eprintln!(
            "[dcmp] layout src={src:#x}+{src_len} dst={dst:#x}+{} work={work:#x} dcmp={dcmp_at:#x} stack={stack:#x}",
            header.unpacked_len
        );
    }
    if std::env::var_os("AD_DCMP_DEBUG").is_some() {
        ...
        eprintln!(
            "[dcmp] work head={:02x?}",
            &work_bytes[..16.min(work_bytes.len())]
        );
    }
