...
fn low_memory_globals_have_sane_defaults() {
    ...
    assert_ne!(
        m.read_u32(globals::TIME),
        0,
        "clock should be fixed, not zero"
    );
...
fn arena_fits_everything_the_runtime_allocates() {
    ...
    let scratch = m
        ...
        .get();
    assert!(
        m.arena_free() > 32 * 1024,
        "no headroom left for structures"
    );
...
fn a_failed_new_handle_leaves_no_orphaned_data_block() {
    ...
    for _ in 0..16 {
        assert_eq!(
            m.new_handle(4096, false),
            0,
            "must fail once slots are gone"
        );
    }
    assert_eq!(
        m.free_bytes(),
